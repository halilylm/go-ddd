...
import (
	...
	"github.com/halilylm/go-ddd/domain/product"
	"testing"
)

func TestMemoryProductRepository_Add(t *testing.T) {
	repo := New()
	product, err := aggregate.NewProduct("Beer", "Good for you're health", 2)
	if err != nil {
		t.Error(err)
	}
...
func TestMemoryProductRepository_Get(t *testing.T) {
	repo := New()
	existingProd, err := aggregate.NewProduct("Beer", "Good for you're health", 2)
	if err != nil {
		t.Error(err)
	}
